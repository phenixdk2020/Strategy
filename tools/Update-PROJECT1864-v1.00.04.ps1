...
.DESCRIPTION
    ...
    (.meta, packages-lock.json, ProjectSettings/*.asset, SceneTemplateSettings.json og PrototypeBattle-scenen),
...
.CHANGELOG
    1.00.04
    - Unity 6.6 bootstrap-whitelist udvidet med ProjectSettings/SceneTemplateSettings.json.
    - Fejloutput viser nu specifikt hvilke lokale ændringer der blokerer auto-stash.
    - Bevarer princippet om kun at auto-stashe kendte UNTRACKED Unity-filer; tracked eller ukendte filer stopper sync.
...
function Test-KnownUnityBootstrapPath {
    param(
        [Parameter(Mandatory)]
        [string]$Path
    )
    ...
    return (
        $normalizedPath -match '^Assets/(?:.+/)?[^/]+\.meta$' -or
        $normalizedPath -match '^Assets/Scenes/.+\.unity$' -or
        $normalizedPath -eq 'Packages/packages-lock.json' -or
        $normalizedPath -match '^ProjectSettings/[^/]+\.asset$' -or
        $normalizedPath -eq 'ProjectSettings/SceneTemplateSettings.json'
    )
}

function Get-NonAutoStashChanges {
    param(
        [Parameter(Mandatory)]
        [string[]]$StatusLines
    )
    ...
    foreach ($line in $StatusLines) {
        ...
        if ($code -ne '??' -or -not (Test-KnownUnityBootstrapPath -Path $path)) {
            $blocked.Add($line)
        }
    }
...
function Test-UnityBootstrapOnly {
    ...
    $blocked = @(Get-NonAutoStashChanges -StatusLines $StatusLines)
    return ($blocked.Count -eq 0)
}
...
try {
    ...
    if ($hasLocalChanges) {
        ...
        if ($unityBootstrapOnly -and -not $NoUnityAutoStash) {
            ...
            if (-not $unityBootstrapOnly) {
                $blockedChanges = @(Get-NonAutoStashChanges -StatusLines $status)
                if ($blockedChanges.Count -gt 0) {
                    Write-Host 'Ændringer der specifikt blokerer auto-stash:'
