...
// v00.00.09f1 backport: manual PlayerCommander route policy/recovery.
// Applies ONLY while Officer AI is disabled. Manual routes march in Column until
// within 14 m of the active waypoint OR an enemy is within EffectiveRange + 12 m.
// If a route makes no useful progress for 3.5 seconds, one controlled bypass waypoint
// is inserted into the existing PlayerCommander route. Navigation V3 remains steering owner.
...
public sealed class PrototypeManualRouteRecovery09H4 : MonoBehaviour
{
    ...
    private static void AutoCreate()
    {
        ...
        GameObject root = new GameObject("PrototypeManualRouteRecovery_v000009f1");
    ...
    private void Awake()
    {
        ...
        if (routesField == null)
        {
            Debug.LogWarning("MANUAL-ROUTE-09F1|Installed=False|Reason=PlayerCommander.routes_not_found");
            enabled = false;
            return;
        }

        Debug.Log(
            "MANUAL-ROUTE-09F1|Installed=True|AI=OFF-only|March=Column|DeployDistance=14|" +
            "EnemyDeploy=EffectiveRange+12|StallRecovery=3.5s|V3SteeringOwner=True");
    }
    ...
    private void ProcessManualRoute(Regiment regiment, object route)
    {
        ...
        float nearestEnemyDistance = FindNearestEnemyDistance(regiment);
        ApplyManualFormation(
            regiment,
            route,
            finalFormationField,
            distance,
        ...
        if (state.RecoveryCount >= MaxRecoveryWaypoints)
        {
            ...
            Debug.LogWarning(string.Format(
                "MANUAL-ROUTE-09F1|Unit={0}|Recovery=False|Reason=RecoveryLimit|Target=({1:0.0},{2:0.0})|Distance={3:0.0}",
        ...
        if (!TryChooseBypass(position, target, out bypass, out score))
        {
            ...
            Debug.LogWarning(string.Format(
                "MANUAL-ROUTE-09F1|Unit={0}|Recovery=False|Reason=NoSafeBypass|ReissueTarget=True|Target=({1:0.0},{2:0.0})",
                regiment.RegimentName,
                target.x,
                target.z));
            return;
        ...
        Debug.LogWarning(string.Format(
            "MANUAL-ROUTE-09F1|Unit={0}|Recovery=True|Reason=NoProgress|InsertedBypass=({1:0.0},{2:0.0})|OriginalTarget=({3:0.0},{4:0.0})|Score={5:0.0}|RecoveryCount={6}",
            regiment.RegimentName,
            bypass.x,
            bypass.z,
            target.x,
    ...
    private static void ApplyManualFormation(
        ...
        RouteProgress state)
    {
        float enemyDeployDistance = regiment.EffectiveRange + DeploymentBuffer;
        bool enemyNear = enemyDistance <= enemyDeployDistance;
        bool destinationNear = destinationDistance <= DeployDistance;

        if (!destinationNear && !enemyNear)
        {
            ...
            if (!state.MarchColumn)
            {
                ...
                Debug.Log(string.Format(
                    "MANUAL-FORMATION-09F1|Unit={0}|MarchColumn=True|DestinationDistance={1:0.0}|EnemyDistance={2:0.0}",
                    regiment.RegimentName,
                    destinationDistance,
                    enemyDistance));
            }
        ...
        if (state.MarchColumn)
        {
            ...
            Debug.Log(string.Format(
                "MANUAL-FORMATION-09F1|Unit={0}|MarchColumn=False|Deploy={1}|DestinationDistance={2:0.0}|EnemyDistance={3:0.0}|Reason={4}",
                regiment.RegimentName,
                finalFormation,
                destinationDistance,
                enemyDistance,
                enemyNear ? "ENEMY_NEAR" : "DESTINATION_NEAR"));
        }
    }

    private static float FindNearestEnemyDistance(Regiment regiment)
    {
