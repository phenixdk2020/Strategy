...
/// <summary>
/// Campaign3 v00.00.10n7c - CITY ASSET RESET.
///
/// Fixes:
/// - no dependency on Unity Resources import for city art,
/// - embedded validated Proposal-3 PNG data for A/B/C,
/// - proactive purge of every older CITY_ART_/CITY_ICON_ child,
/// - materially smaller city footprint,
/// - county/Amt systems remain deliberately disabled (Way 1).
/// </summary>
[DefaultExecutionOrder(32200)]
public sealed class CampaignMapOnlyCityArtV010N7C : MonoBehaviour
{
    private const string RootName = "CITY_ART_PROPOSAL3_10N7C";
    ...
    {
        if (!CampaignBuildInfo.IsCurrentVersion("v00.00.10n7c") &&
            !CampaignBuildInfo.IsCurrentVersion("v00.00.10n7d"))
            return;

        if (Object.FindAnyObjectByType<CampaignMapOnlyCityArtV010N7C>() != null)
            return;

        GameObject go = new GameObject("PROJECT1864_MAP_ONLY_CITY_ART_10N7C");
        DontDestroyOnLoad(go);
        go.AddComponent<CampaignMapOnlyCityArtV010N7C>();
    }
    ...
    private bool EnsureEmbeddedTexturesAndMaterials()
    {
        ...
        if (materialA == null)
            materialA = CreateMaterial(textureA, "CITY_ART_A_10N7C");
        if (materialB == null)
            materialB = CreateMaterial(textureB, "CITY_ART_B_10N7C");
        if (materialC == null)
            materialC = CreateMaterial(textureC, "CITY_ART_C_10N7C");
    ...
    private static Mesh BuildBottomAnchoredQuad()
    {
        Mesh mesh = new Mesh
        {
            name = "CITY_ART_BOTTOM_ANCHORED_QUAD_10N7C"
        };
