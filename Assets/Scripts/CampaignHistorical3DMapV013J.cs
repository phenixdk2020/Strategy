...
// Presentation-only rebuild on top of the v13i DEM cache.
...
public sealed class CampaignHistorical3DMapV013J : MonoBehaviour
{
    ...
    {
        if (!string.Equals(SceneManager.GetActiveScene().name, "CampaignMap", StringComparison.Ordinal)) return;
        if (UnityEngine.Object.FindAnyObjectByType<CampaignHistorical3DMapV013J>() != null) return;
        new GameObject("CampaignHistorical3DMapV013J").AddComponent<CampaignHistorical3DMapV013J>();
    }
    ...
    private IEnumerator BuildHistoricalMap()
    {
        ...
        if (v13i != null) v13i.enabled = false;
        ready = true;
    ...
    private bool IsV13IReady()
    {
        const BindingFlags flags = BindingFlags.Instance | BindingFlags.NonPublic;
        FieldInfo a = typeof(CampaignDenmarkGisFoundationV013I).GetField("terrainReady", flags);
        FieldInfo b = typeof(CampaignDenmarkGisFoundationV013I).GetField("finishingPassDone", flags);
        return a != null && b != null && (bool)a.GetValue(v13i) && (bool)b.GetValue(v13i);
    }
    ...
    private void ResolveDenmarkRings()
    {
        ...
        if (denmarkRings == null) { ringBounds = new RingBounds[0]; return; }
        ...
        for (int r = 0; r < denmarkRings.Length; r++)
        {
            ...
            foreach (Vector2 p in denmarkRings[r])
            {
    ...
    {
        int width = GridX + 1, height = GridZ + 1, count = width * height;
        ...
        for (int z = 0; z < height; z++)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                int i = z * width + x;
                land[i] = IsDenmarkLand(lat, lon) && !IsHydrologyWater(lat, lon);
                elevation[i] = land[i] ? Mathf.Clamp(SampleElevationMeters(lat, lon), 0f, 220f) : 0f;
                uv[i] = new Vector2(ux, vz);
            }
            if ((z & 15) == 0)
            {
                status = "bygger glat 3D-kort " + Mathf.RoundToInt(vz * 100f) + "%";
                yield return null;
            }
        }
        ...
        for (int z = 0; z < height; z++)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                double lon = MinLon + (MaxLon - MinLon) * ux;
                int i = z * width + x;
                vertices[i] = CampaignGeoProjection.Project3D(lat, lon, WaterY + 0.05f + elevation[i] * ElevationToUnity);
            }
        }
        ...
        for (int z = 0; z < GridZ; z++)
        {
            ...
            {
                int i0 = z * width + x, i1 = i0 + 1, i2 = i0 + width, i3 = i2 + 1;
                if (!(land[i0] && land[i1] && land[i2] && land[i3])) continue;
                triangles.Add(i0); triangles.Add(i2); triangles.Add(i1);
                triangles.Add(i1); triangles.Add(i2); triangles.Add(i3);
            }
        }

        Mesh mesh = new Mesh { name = "V013J_Denmark_Mainland_Smooth_Mesh", indexFormat = IndexFormat.UInt32 };
    ...
    private static void SmoothElevation(float[] values, bool[] land, int width, int height, int passes, float strength)
    {
        ...
        for (int pass = 0; pass < passes; pass++)
        {
            ...
            for (int z = 1; z < height - 1; z++)
            {
                for (int x = 1; x < width - 1; x++)
                {
                    int i = z * width + x;
                    if (!land[i]) continue;
                    float sum = values[i]; int n = 1;
                    int a = i - 1, b = i + 1, c = i - width, d = i + width;
                    if (land[a]) { sum += values[a]; n++; }
                    if (land[b]) { sum += values[b]; n++; }
                    if (land[c]) { sum += values[c]; n++; }
                    if (land[d]) { sum += values[d]; n++; }
    ...
    {
        terrainTiles.Clear();
        int xMin = LonToTileX(MinLon, TerrainZoom), xMax = LonToTileX(MaxLon, TerrainZoom);
        int yMin = LatToTileY(MaxLat, TerrainZoom), yMax = LatToTileY(MinLat, TerrainZoom);
        for (int y = yMin; y <= yMax; y++)
        {
            for (int x = xMin; x <= xMax; x++)
            {
                ...
                try
                {
                    ...
                    terrainTiles[TileKey(x, y)] = new TerrainTile(t.width, t.height, t.GetPixels32());
                    Destroy(t);
                }
                catch (Exception ex) { Debug.Log("CAMPAIGN-V013J|TerrainCacheRead=False|" + ex.Message); }
            }
        }
    }
    ...
    private float SampleElevationMeters(double lat, double lon)
    {
        ...
        int tileX = (int)Math.Floor(tx), tileY = (int)Math.Floor(ty);
        TerrainTile tile;
        if (!terrainTiles.TryGetValue(TileKey(tileX, tileY), out tile)) return FallbackElevationMeters(lat, lon);

        float u = (float)(tx - tileX);
        float v = (float)(ty - tileY);
        float px = Mathf.Clamp(u * (tile.Width - 1), 0f, tile.Width - 1);
        // Unity pixel arrays use bottom-left origin; slippy tile v=0 is north/top.
        float py = Mathf.Clamp((1f - v) * (tile.Height - 1), 0f, tile.Height - 1);
        int x0 = Mathf.FloorToInt(px), y0 = Mathf.FloorToInt(py);
        int x1 = Mathf.Min(x0 + 1, tile.Width - 1), y1 = Mathf.Min(y0 + 1, tile.Height - 1);
    ...
    private static float DecodeTerrarium(Color32 c) { return c.r * 256f + c.g + c.b / 256f - 32768f; }

    private static float FallbackElevationMeters(double lat, double lon)
    {
        float nx = (float)((lon - MinLon) * 0.72), nz = (float)((lat - MinLat) * 0.72);
        float broad = Mathf.PerlinNoise(nx, nz), fine = Mathf.PerlinNoise(nx * 2.1f + 13.1f, nz * 2.1f + 7.4f);
    ...
    private void CreateMaterials()
    {
        ...
        if (mapTexture == null) mapTexture = CreateProceduralCartographicTexture(1024, 768);
        Shader shader = Shader.Find("Universal Render Pipeline/Lit");
        if (shader == null) shader = Shader.Find("Standard");
        if (shader == null) shader = Shader.Find("Unlit/Texture");
        terrainMaterial = new Material(shader) { name = "V013J_CartographicTerrain", color = Color.white };
    ...
    private Texture2D TryLoadHistoricalTexture()
    {
        ...
        try
        {
            Texture2D t = new Texture2D(2, 2, TextureFormat.RGBA32, false, false);
            t.LoadImage(File.ReadAllBytes(path), false);
        ...
        catch (Exception ex) { Debug.LogWarning("CAMPAIGN-V013J|HistoricalTexture=False|" + ex.Message); return null; }
    }

    private static Texture2D CreateProceduralCartographicTexture(int width, int height)
    {
        Texture2D t = new Texture2D(width, height, TextureFormat.RGBA32, false, false)
        {
        ...
        Color low = new Color(0.62f, 0.67f, 0.47f), mid = new Color(0.55f, 0.62f, 0.42f), high = new Color(0.72f, 0.68f, 0.49f);
        for (int y = 0; y < height; y++)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                float broad = Mathf.PerlinNoise(u * 8f + 11.3f, v * 8f + 4.7f);
                float fine = Mathf.PerlinNoise(u * 34f + 2.4f, v * 34f + 19.1f);
                Color c = Color.Lerp(low, mid, broad);
                c = Color.Lerp(c, high, Mathf.Clamp01((fine - 0.58f) * 1.5f));
                c *= 0.96f + (fine - 0.5f) * 0.05f;
                pixels[y * width + x] = c;
            }
        }
    ...
    private void BuildSmallSettlements()
    {
        foreach (KeyValuePair<string, CampaignNodeState> pair in CampaignSession.Nodes)
        {
            ...
            if (node == null || node.Region != CampaignMapRegion.Denmark || node.Id == "AALBORG") continue;
            ...
            cityVisuals.Add(new CityVisual(visual, major));
        }
    }
    ...
    private void BuildAalborgHarbourAndBarracks()
    {
        ...
        Vector3 p = CampaignGeoProjection.Project3D(node.Latitude, node.Longitude, 0f);
        p.y = SampleGroundHeight(p.x, p.z) + 0.025f;
        cityVisuals.Add(new CityVisual(CreateMiniTown("V013J_City_AALBORG", p, 5, 0.13f, true), true));

        Transform harbour = new GameObject("V013J_Aalborg_Harbour").transform;
        harbour.SetParent(constructionRoot, false);
        harbour.position = CampaignGeoProjection.Project3D(57.055, 9.920, WaterY + 0.04f);
        ...
        barracks.SetParent(constructionRoot, false); barracks.position = bp;
    ...
    private void BuildAarhusFarm()
    {
        ...
        farm.SetParent(constructionRoot, false); farm.position = p;
    ...
    {
        GameObject go = new GameObject(name); go.transform.SetParent(cityRoot, false); go.transform.position = centre;
        for (int i = 0; i < houses; i++)
        {
            float angle = i * 2.399963f, radius = 0.08f + Mathf.Sqrt(i + 1) * 0.035f;
            CreateHouse(go.transform, "House_" + i, new Vector3(Mathf.Cos(angle) * radius, 0f, Mathf.Sin(angle) * radius), scale);
        }
        ...
        if (port) CreateBox(go.transform, "Pier", new Vector3(scale * 1.1f, 0.01f, scale * 0.55f), new Vector3(scale * 1.4f, 0.015f, scale * 0.16f), harborMaterial);
        return go;
    }
    ...
    private void CreateHouse(Transform parent, string name, Vector3 local, float scale)
    {
        ...
        roof.name = name + "_Roof"; roof.transform.SetParent(parent, false);
        ...
        roof.GetComponent<Renderer>().sharedMaterial = cityRoofMaterial; RemoveCollider(roof);
    }
    ...
    private void HideLegacyOversizedVisuals()
    {
        ...
        if (v13iRoot == null) return;
        foreach (LineRenderer line in v13iRoot.GetComponentsInChildren<LineRenderer>(true))
            if (line != null && line.name.StartsWith("GIS_Coast_", StringComparison.Ordinal)) line.enabled = false;
    }
    ...
    private void HideOldTerrainTiles()
    {
        GameObject v13iRoot = GameObject.Find("V013I_DENMARK_GIS_FOUNDATION");
        if (v13iRoot == null) return;
        foreach (MeshRenderer r in v13iRoot.GetComponentsInChildren<MeshRenderer>(true))
            if (r != null && r.gameObject.name.StartsWith("GIS_Terrain_", StringComparison.Ordinal)) r.enabled = false;
        foreach (MeshCollider c in v13iRoot.GetComponentsInChildren<MeshCollider>(true))
            if (c != null && c.gameObject.name.StartsWith("GIS_Terrain_", StringComparison.Ordinal)) c.enabled = false;
    }
    ...
    private void RetuneExistingInfrastructure()
    {
        ...
        foreach (LineRenderer line in v13iRoot.GetComponentsInChildren<LineRenderer>(true))
        {
            ...
            if (line.name.StartsWith("GIS_Road_", StringComparison.Ordinal)) line.widthMultiplier = 0.055f;
            else if (line.name.StartsWith("GIS_Ferry_", StringComparison.Ordinal)) line.widthMultiplier = 0.045f;
        }
    }
    ...
    private void ConfigureCamera()
    {
        ...
        cam.nearClipPlane = 0.02f; cam.fieldOfView = 48f;
        CampaignMapCameraController c = cam.GetComponent<CampaignMapCameraController>();
        if (c == null) return;
        c.UseLegacyTerrainFloor = false; c.TerrainClearance = 0.70f; c.MinHeight = 0.70f; c.MaxHeight = 230f;
        c.PanSpeed = 38f; c.ZoomSpeed = 125f; c.MinPitch = 22f; c.MaxPitch = 78f; c.ZoomTowardCursor = true; c.MiddleMousePan = true;
    }
    ...
    private void UpdateCityLod()
    {
        ...
        foreach (CityVisual city in cityVisuals)
            if (city.Root != null) SetRenderersVisible(city.Root, h < 52f || city.Major);
    }

    private static float SampleGroundHeight(float x, float z)
    {
        float best = float.NegativeInfinity;
        foreach (RaycastHit hit in Physics.RaycastAll(new Vector3(x, 1000f, z), Vector3.down, 2000f))
        {
            if (hit.collider == null || !string.Equals(hit.collider.gameObject.name, "V013J_SmoothTerrain", StringComparison.Ordinal)) continue;
            if (hit.point.y > best) best = hit.point.y;
        }
        return float.IsNegativeInfinity(best) ? WaterY + 0.08f : best;
    }

    private bool IsDenmarkLand(double lat, double lon)
    {
        if (denmarkRings == null || denmarkRings.Length == 0) return lat >= MinLat && lat <= MaxLat && lon >= MinLon && lon <= MaxLon;
        ...
        for (int i = 0; i < denmarkRings.Length; i++)
        {
            ...
            if (PointInPolygon(p, denmarkRings[i])) return true;
        }
        return false;
    }
    ...
    {
        foreach (HydroLine h in hydrology) if (DistanceToPolylineKm(lat, lon, h.Points) <= h.WidthKm * 0.5f) return true;
        return false;
    }
    ...
    private static bool PointInPolygon(Vector2 point, Vector2[] polygon)
    {
        ...
        for (int i = 0, j = polygon.Length - 1; i < polygon.Length; j = i++)
        {
            ...
            bool cross = ((a.y > point.y) != (b.y > point.y)) && (point.x < (b.x - a.x) * (point.y - a.y) / ((b.y - a.y) == 0f ? 0.000001f : (b.y - a.y)) + a.x);
    ...
    private static float DistanceToPolylineKm(double lat, double lon, Vector2[] line)
    {
        ...
        if (line == null || line.Length < 2) return best;
        for (int i = 0; i < line.Length - 1; i++)
        {
            double lat0 = line[i].y, lon0 = line[i].x, lat1 = line[i + 1].y, lon1 = line[i + 1].x;
            double cos = Math.Cos(((lat + lat0 + lat1) / 3.0) * Math.PI / 180.0);
    ...
    private static string GetTerrainCachePath(int zoom, int x, int y) { return Path.Combine(Application.persistentDataPath, "PROJECT1864", "TerrainCache", "terrarium", zoom.ToString(), x.ToString(), y + ".png"); }
    private static int LonToTileX(double lon, int zoom) { return (int)Math.Floor((lon + 180.0) / 360.0 * Math.Pow(2.0, zoom)); }
    private static int LatToTileY(double lat, int zoom) { double r = lat * Math.PI / 180.0, n = Math.Pow(2.0, zoom); return (int)Math.Floor((1.0 - Math.Asinh(Math.Tan(r)) / Math.PI) * 0.5 * n); }

    private static Transform CreateChild(Transform parent, string name) { Transform t = new GameObject(name).transform; t.SetParent(parent, false); return t; }
    private static Material CreateLitMaterial(Color color, string name) { Shader s = Shader.Find("Universal Render Pipeline/Lit") ?? Shader.Find("Standard") ?? Shader.Find("Unlit/Color"); Material m = new Material(s) { name = name, color = color }; if (m.HasProperty("_Smoothness")) m.SetFloat("_Smoothness", 0.02f); if (m.HasProperty("_Metallic")) m.SetFloat("_Metallic", 0f); return m; }
    private static void CreateBox(Transform parent, string name, Vector3 pos, Vector3 scale, Material mat) { GameObject g = GameObject.CreatePrimitive(PrimitiveType.Cube); g.name = name; g.transform.SetParent(parent, false); g.transform.localPosition = pos; g.transform.localScale = scale; g.GetComponent<Renderer>().sharedMaterial = mat; RemoveCollider(g); }
    private static void RemoveCollider(GameObject g) { Collider c = g.GetComponent<Collider>(); if (c != null) Destroy(c); }
    private static void HideObjectTree(string name) { GameObject g = GameObject.Find(name); if (g != null) SetRenderersVisible(g, false); }
    private static void SetRenderersVisible(GameObject g, bool visible) { if (g == null) return; foreach (Renderer r in g.GetComponentsInChildren<Renderer>(true)) if (r != null) r.enabled = visible; }

    private void OnGUI()
    {
        if (statusStyle == null) { statusStyle = new GUIStyle(GUI.skin.label) { fontSize = 9, alignment = TextAnchor.LowerLeft }; statusStyle.normal.textColor = new Color(0.82f, 0.84f, 0.77f); }
        GUI.Label(new Rect(12f, Screen.height - 24f, 900f, 18f), "v13j · " + status, statusStyle);
    }

    private sealed class TerrainTile { public readonly int Width, Height; public readonly Color32[] Pixels; public TerrainTile(int w, int h, Color32[] p) { Width = w; Height = h; Pixels = p; } }
    private sealed class HydroLine { public readonly float WidthKm; public readonly Vector2[] Points; public HydroLine(float w, Vector2[] p) { WidthKm = w; Points = p; } }
    private struct RingBounds { public readonly float MinX, MaxX, MinY, MaxY; public RingBounds(float a, float b, float c, float d) { MinX = a; MaxX = b; MinY = c; MaxY = d; } }
    private sealed class CityVisual { public readonly GameObject Root; public readonly bool Major; public CityVisual(GameObject root, bool major) { Root = root; Major = major; } }
}
